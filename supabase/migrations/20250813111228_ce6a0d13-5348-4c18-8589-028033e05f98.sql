-- Adicionar jobs agendados para automação bancária e sistema de filas
...
-- Criar alertas automáticos para monitoramento
...
(
  ...
  'warning',
  'pending_tasks',
  'Fila com muitas tarefas pendentes',
  100,
  0,
  '{"check_frequency": "5min", "escalate_after": "30min"}'::jsonb
),
(
  'worker_offline',
  'critical',
  'offline_workers',
  'Workers offline detectados',
  1,
  0,
  '{"check_frequency": "5min", "auto_restart": true}'::jsonb
),
(
  'bank_ingest_failure',
  'warning',
  ...
  3,
  0,
  '{"time_window": "24h", "notify_accountant": true}'::jsonb
);
